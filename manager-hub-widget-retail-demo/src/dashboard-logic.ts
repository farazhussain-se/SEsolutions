// ============================================================================
// MANAGER HUB — DASHBOARD LOGIC (~2600 lines: this is the whole widget)
// ============================================================================
//
// DEV — architecture in one paragraph: this file has no framework, no
// virtual DOM, and no build-time templating. `initDashboard()` (below) is
// called once, from a React `useEffect` in manager-hub-widget.tsx, with a
// plain `container` HTMLElement whose innerHTML is the static markup in
// dashboard.html.txt. Every `initX(container, ...)` function in this file
// does the same three things for one feature/tab: (1) `querySelector` the
// bits of that static markup it owns, (2) fetch real data from Staffbase
// (Journeys/Tasks/Notifications/Users/Groups) via `staffbaseFetch()`, and
// (3) wire up click/change listeners and re-render on demand. There's no
// component tree to reason about — just "find these ids, fill them in,
// listen for clicks." `demoMode` (a Studio config toggle) controls whether
// each feature's hardcoded BASELINE_* sample data is blended in alongside
// whatever the live API actually returns, so the dashboard is never empty
// even on a brand-new branch with no Journeys/Tasks/Requisitions set up yet.
//
// PM — what a manager can actually do here, tab by tab:
//   • Action Center      — approve time-off/shift-swap requests, resolve pay
//                          discrepancies, review status-change approvals, and
//                          triage HR tickets. (Currently illustrative/static —
//                          there's no live payroll or ticketing system wired
//                          in yet; see each subview's comment for specifics.)
//   • My Tasks           — the signed-in manager's own real assigned tasks,
//                          pulled live from Staffbase Tasks.
//   • Team Readiness     — real employee progress through Staffbase Journeys
//                          (onboarding, certifications, promotions, etc.) and
//                          real open Tasks assigned to the team, each with
//                          filters, sorting, and one-click reminders that
//                          create a real task and/or send a real push
//                          notification to the employee.
//   • Roster & Leave     — today's shift coverage and leave-of-absence
//                          tracking, and approving shift-swap/coverage
//                          requests submitted by the standalone employee-
//                          facing shift-roster widget (shared via
//                          localStorage — see shift-data.ts).
//   • Talent & Growth    — submit a real role/title change for a team member
//                          (sends them a real notification) and see who's
//                          ready for their next role.
//   • Requisitions       — a Workday-style purchase-requisition flow: create
//                          one, route it to the real Facilities/Maintenance
//                          group in Staffbase, and track it through Draft →
//                          PO Issued → Received → Invoiced. Persisted in the
//                          browser's localStorage — there's no real Workday
//                          or procurement backend behind this, it's a
//                          convincing mock of that workflow.
//   • Setup (small gear) — the one-time API token field, a "Demo mode"
//                          on/off switch, the signed-in manager's real
//                          identity, and a built-in diagnostics check.
//
// Every feature above that says "real" calls the actual Staffbase Branch API
// directly from the browser (see staffbaseFetch below) using the token
// entered once in this widget's Studio configuration — there is no separate
// backend server for this widget to stand up, tunnel, or maintain.
...
// DEV: every Tasks API call in this file is scoped to one Tasks *plugin
// installation* on a branch (there can be more than one). There's no way to
// discover "the" installation generically — it has to be looked up once per
// branch (GET /branch/installations, filter pluginId === "tasks") and hardcoded
// here, or threaded through as a config value the way apitoken/demomode are.
// This id is left over from the original branded build; replace it with the
// real installation id for whatever branch this demo is installed on, or
// nothing under Team Tasks / My Tasks / Requisitions (all of which depend on
// it) will resolve.
//
// PM: this is the one piece of setup this "generic" demo does NOT do
// automatically — unlike the API token (one Studio field) and the branch URL
// (auto-detected), the specific Tasks installation still has to be looked up
// and pasted in here by whoever stands the demo up on a new branch.
const DEFAULT_TASKS_INSTALLATION_ID = "REPLACE_ME_TASKS_INSTALLATION_ID";

// DEV: the real Staffbase Tasks app's content page for this branch — distinct
// from the Tasks *plugin installation* id used everywhere else in this file
// (that one drives the /tasks/{id}/... API; this one is the browsable content
// node a manager lands on in Staffbase itself). There's no documented
// per-task deep link on this API, so every "open" affordance below points
// here rather than claiming a precision it can't deliver — good enough to let
// a manager actually open and edit the real task, which is the point.
//
// PM: this content-node id is genuinely branch-specific — it's the internal
// Staffbase page ID for wherever the Tasks app lives on the customer's site,
// and there's no API to look it up automatically. Whoever installs this demo
// on a new branch needs to open their own Tasks app once, copy the id out of
// its URL, and paste it in below. The hostname portion is NOT hardcoded here
// (see openManagerTasksApp) — only this one id needs replacing per install.
const MANAGER_TASKS_APP_CONTENT_ID = "REPLACE_ME_TASKS_CONTENT_NODE_ID";

function openManagerTasksApp(branchBase: string): void {
  const host = branchBase.replace(/\/api\/?$/, "");
  window.open(`${host}/content/tasks/${MANAGER_TASKS_APP_CONTENT_ID}`, "_blank", "noopener");
}
...
/**
 * DEV: single entry point mounted by the React wrapper in a useEffect.
 * Everything here queries within `container` (not `document`) so multiple
 * instances on one page wouldn't collide — the one known limitation carried
 * over from the original static file is that IDs are still assumed unique
 * per instance, same tradeoff made for servicenow-widget. Below is every
 * feature this widget wires up, in the order it initializes them — see the
 * file-level comment at the top of this file for what each one does from a
 * product perspective.
 */
export function initDashboard(opts: DashboardOptions): void {
  ...
  wireOpenTasksLinks(container, branchBase);
...
function wireOpenTasksLinks(container: HTMLElement, branchBase: string): void {
  ["#myTasksOpenLink", "#tasksOpenLink"].forEach((sel) => {
    container.querySelector<HTMLElement>(sel)?.addEventListener("click", () => openManagerTasksApp(branchBase));
  });
}
...
/** DEV: the only real "setup" step: is a token configured, and does it
 * actually authenticate? A cheap GET /branch call proves both at once.
 * PM: this is the badge on the Setup tab that says Connected/Invalid/Not
 * configured — the fastest way to tell whether the one required install
 * step (pasting a Staffbase API token) actually worked. */
 ...
 * PM: this is the "Run Diagnostics" button — a self-check anyone can run
 * after installing this widget on a new branch, without needing a developer,
 * to confirm the token can actually reach every feature this widget uses. */
...
const BASELINE_EMPLOYEES_TEMPLATE: JourneyEmployeeEntry[] = [
  ...
  { name: "Natalie Cho", initials: "NC", journeyName: "Inter-Store Transfer", stepIndex: 1, stepName: "Receiving Store Confirmation", totalSteps: 4, completed: false, daysOnStep: 6 },

  // ASM/SM Leadership Pipeline
  { name: "Daniel Esposito", initials: "DE", journeyName: "ASM/SM Leadership Pipeline", stepIndex: 2, stepName: "Store P&L Training", totalSteps: 4, completed: false, daysOnStep: 1 },
  ...
  { name: "Bradley Simms", initials: "BS", journeyName: "Workplace Incident & Injury (Workers' Comp)", stepIndex: 2, stepName: "Light-Duty Restrictions Coordinated with ASM", totalSteps: 4, completed: false, daysOnStep: 4 },
...
/** DEV: real POST to the Tasks API. PM: this is what actually happens when a
 * manager clicks "Remind" on an overdue journey step — it doesn't just show a
 * toast, it creates a genuine task assigned to that employee in Staffbase
 * Tasks (see remindJourneyPerson below), so the reminder shows up in the
 * employee's own task list, not just as a notification they can dismiss. */
...
/**
 * DEV: owns the whole "Team Readiness → Journeys" sub-tab: fetches real
 * Journey progress (fetchJourneysEmployees), blends in BASELINE_EMPLOYEES
 * when demoMode is on, wires the type/sort/overdue filter bar, and wires the
 * "Remind" buttons (see remindJourneyPerson) to create a real task + send a
 * real notification. Returns a `setOverdueFilterActive` handle so the
 * top-of-dashboard "Overdue Journey Steps" metric card can deep-link
 * straight into this tab with the filter already applied.
 *
 * PM: this answers "who on my team is behind on onboarding, certifications,
 * or any other multi-step process, and by how long" — using Staffbase's real
 * Journeys data, not a static roster.
 */
...
const BASELINE_OVERDUE_TASKS: OverdueTaskEntry[] = [
  ...
  { name: "Zack Hall", title: "Complete annual safety recertification", daysOverdue: 9 },
];
...
/**
 * DEV: owns "Team Readiness → Team Tasks" — real open Tasks grouped by real
 * assignee, its own filter/sort bar, an overdue-tasks sub-panel with
 * notification-only reminders (remindTaskPerson — no new task is created,
 * since an overdue *task* already exists as one), and it also exposes
 * `getUserId(name)`, which the Promotion Launcher (initPromotionLauncher,
 * below) depends on to resolve a selected name back to a real Staffbase user
 * id before it can send that person a real notification.
 *
 * PM: this is the real, live "what does my team actually have open right
 * now, and what's overdue" view — plus the data feed that powers the
 * "who can I change the role of" picker elsewhere on the Talent tab.
 */
function initTeamTasksAndRoleChange(
  ...
  const BASELINE_TASKS: TaskGroup[] = demoMode
    ? [
        { name: "Jamie Cole", tasks: [{ title: "Submit annual safety recertification documentation", priority: "Priority_1", status: "Open" }] },
        { name: "Priya Shah", tasks: [{ title: "Shadow 2 opening shifts with Checkout Lead", priority: "Priority_3", status: "Open" }] },
  ...
  const BASELINE_MEMBERS: RoleChangeMember[] = demoMode
    ? [
        { name: "Kristina Crawford", title: "Checkout Associate" },
        { name: "Gail Gonzalez", title: "Guest Services Lead" },
        { name: "Billie Kelley", title: "Guest Services Lead" },
        { name: "Greg Russell", title: "Guest Services Lead" },
      ]
    : [];
  ...
  function renderTaskGroups(groups: TaskGroup[]) {
    ...
    groups.forEach((g) => {
      ...
      g.tasks.forEach((t) => {
        ...
        item.addEventListener("click", () => openManagerTasksApp(branchBase));
        const pill = item.querySelector(".cw-pill")!;
        if (t.daysOverdue) {
...
interface Worktags {
  costCenter: string; // e.g. "CC-4102 · Downtown – Facilities"
  spendCategory: string; // e.g. "SC-REPAIRS-BLDG · Repairs & Maintenance – Building"
  location: string; // e.g. "Northwind Retail – Downtown"
}
...
interface Requisition {
  ...
  requester: string;
  company: string; // "Northwind Retail, Inc."
  ...
  assignedTeam?: string; // routing target, e.g. "Store Facilities & Maintenance Team"
}
...
// Facilities Operations routing target — resolved by matching this display
// title against the branch's real Staffbase groups at runtime (see
// resolveGroupIdByTitle below), the same "look it up by title" pattern used
// by other internal demo/rebrand tooling in this repo.
const FACOPS_GROUP_TITLE = "Store Facilities & Maintenance Team";

// Option lists for the create form — Worktag values keep Workday vocabulary.
const REQ_COST_CENTERS = [
  "CC-4102 · Downtown – Facilities",
  "CC-4101 · Downtown – Housekeeping",
  "CC-4108 · Downtown – Point of Sale",
  "CC-4110 · Downtown – Fitting Rooms",
  "CC-4120 · Downtown – Café",
];
const REQ_SPEND_CATEGORIES = [
  ...
  "SC-POS-SUPPLY · Point-of-Sale Supplies",
  "SC-SUPPLIES-JAN · Janitorial & Cleaning Supplies",
  "SC-SUPPLIES-OFFICE · Office & Admin Supplies",
];
const REQ_LOCATIONS = ["Northwind Retail – Downtown", "Northwind Retail – Eastgate", "Northwind Retail – Riverside Flagship"];
...
const REQUISITION_STORAGE_KEY = "retaildemo-manager-hub:requisitions";
...
// Realistic retail facilities/store requisitions across the status range so
...
const BASELINE_REQUISITIONS: Requisition[] = [
  {
    ...
    supplier: "SafeGuard Fire & Safety Services",
    worktags: {
      costCenter: "CC-4102 · Downtown – Facilities",
      spendCategory: "SC-INSPECTIONS · Safety & Compliance Inspections",
      location: "Northwind Retail – Downtown",
    },
    status: "Draft",
    currency: "USD",
  ...
  },
  {
    ...
    company: "Northwind Retail, Inc.",
    requisitionType: "Non-Catalog",
    supplier: "Meridian Industrial Supply",
    worktags: {
      costCenter: "CC-4102 · Downtown – Facilities",
    ...
    status: "In Progress",
    currency: "USD",
    memo: "Sales-floor HVAC compressor failed; replacement + labor.",
    chainStage: "Requisition",
    lines: [
  ...
  },
  {
    ...
    supplier: "Apex POS Supply Co.",
    worktags: {
      costCenter: "CC-4108 · Downtown – Point of Sale",
      spendCategory: "SC-POS-SUPPLY · Point-of-Sale Supplies",
      location: "Northwind Retail – Downtown",
    },
    status: "Successfully Completed",
    currency: "USD",
    memo: "Monthly point-of-sale supplies restock.",
    chainStage: "Received",
    purchaseOrderNumber: "PO-WD-2026-2087",
    lines: [
      { lineType: "Goods", itemDescription: "Thermal receipt paper rolls (case of 50)", spendCategory: "Point-of-Sale Supplies", quantity: 6, unitOfMeasure: "Case", unitCost: 189.0, extendedAmount: 1134.0 },
      { lineType: "Goods", itemDescription: "Barcode label rolls (case of 20)", spendCategory: "Point-of-Sale Supplies", quantity: 4, unitOfMeasure: "Case", unitCost: 42.5, extendedAmount: 170.0 },
    ],
  },
  {
    ...
    supplier: "Meridian Industrial Supply",
    worktags: {
      costCenter: "CC-4101 · Downtown – Housekeeping",
  ...
  },
  {
    ...
    status: "Sent Back",
    currency: "USD",
    memo: "Returned by ASM: attach the vendor quote and pick a Cost Center.",
    chainStage: "Requisition",
    lines: [
      { lineType: "Service", itemDescription: "Fitting-room wayfinding signage — replace 6 panels", spendCategory: "Repairs & Maintenance – Building", extendedAmount: 540.0 },
    ],
  },
];

/**
 * DEV: owns the whole Requisitions tab — landing card, the Open/Completed
 * tabbed history, a Workday-style detail view (worktags + line items +
 * procurement-chain stepper), and the create form (delegated to
 * initReqCreateForm below). Requisitions are NOT a real Staffbase concept —
 * there's no Workday/procurement backend here — they're stored entirely in
 * this browser's localStorage (loadStoredRequisitions/saveStoredRequisition).
 * The one genuinely real integration point: submitting one can look up and
 * notify a real Staffbase group (resolveGroupIdByTitle + FACOPS_GROUP_TITLE).
 *
 * PM: this simulates a full purchase-requisition approval workflow end to
 * end — good for showing "what this could look like," but every requisition
 * shown (besides ones you create yourself in the demo) is sample data, and
 * nothing here writes back to a real procurement system.
 */
...
const BASELINE_MY_TASKS: MyTaskEntry[] = [
  { title: "Complete quarterly store safety walkthrough", priority: "Priority_1", status: "Open", daysOverdue: 2 },
...
/**
 * DEV: owns "My Tasks" — the *signed-in manager's own* real assigned tasks
 * (fetchMyTasks, above), not their team's. Uses widgetApi.getUserInformation()
 * to know who "me" is, same identity source as the header/Setup tab.
 *
 * PM: lets a manager see and open their own real to-do list from inside this
 * dashboard instead of switching to the separate Tasks app — one less place
 * to check.
 */
...
const ROLE_CHANGE_STORAGE_KEY = "retaildemo-manager-hub:role-changes";
...
/**
 * DEV: owns "Talent & Growth → Initiate Role Change". Its member dropdown is
 * populated from initTeamTasksAndRoleChange's real Tasks-assignee list
 * (roleChangeMembers), and getUserId (also from that function) resolves the
 * selected name to a real user id. The submission itself only writes to
 * localStorage (loadStoredRoleChanges/saveStoredRoleChange) — there's no real
 * "change this person's title" API call — but it DOES send that person a
 * real Staffbase notification confirming the change, if a token + resolved
 * user id are both available.
 *
 * PM: important distinction — clicking "Submit for Approval" does not
 * actually change anyone's title or pay in any real system. It logs the
 * change locally (so the demo has something to show in Action Center →
 * Status Change Approvals) and sends the employee a real heads-up
 * notification. Treat this as a convincing mock of the workflow, not a real
...
// BASELINE_REQUISITIONS. shiftId targets Jordan's own Visual Merchandising
...
/**
 * DEV: owns "Roster & Leave" — the shift-coverage matrix and the shift-swap/
 * coverage/call-off approval list. Reads and writes the SAME localStorage
 * keys as the separate, employee-facing shift-roster widget (see the header
 * comment above shiftTimeRange, and shift-data.ts) — there's no server, so
 * "shared data" here means "shared origin, shared localStorage keys."
 * Approving a request here writes the effect back into that shared roster so
 * the employee-facing widget picks it up too.
 *
 * PM: this is a two-widget story — employees request time off/swaps in their
 * own widget, and a manager approves them here. Both widgets need to be
 * installed on pages the same audience can reach for this to make sense as a
 * demo; this widget alone only shows the manager-facing half.
 */
function initShiftRoster(
  ...
  window.addEventListener("storage", (e) => {
    if (!e.key || e.key.startsWith("retaildemo-shift-roster:")) refresh();
  });
