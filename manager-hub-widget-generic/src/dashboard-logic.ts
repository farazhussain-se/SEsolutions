...
export interface DashboardOptions {
  ...
  /** The Staffbase Tasks app's browsable content-page URL for this branch —
   * distinct from the Tasks *plugin installation* id below (that one drives
   * the /tasks/{id}/... API; this is the content node a manager lands on in
   * Staffbase itself to open/edit a real task). There's no documented
   * per-task deep link on this API, so every "open" affordance in this file
   * points here rather than claiming a precision it can't deliver. No
   * generic default — this is specific to each Staffbase branch, so it must
   * be configured in Studio or the "open in Tasks app" links simply no-op.
   */
  tasksAppUrl?: string;
}

// No generic defaults for either of these — they're specific to one
// Staffbase branch's Tasks installation, so a fresh deployment without
// Studio config just skips Tasks-dependent features gracefully (same
// fallback behavior as an unreachable API) rather than silently pointing at
// someone else's branch.
const DEFAULT_TASKS_INSTALLATION_ID = "";
let managerTasksAppUrl = "";

function openManagerTasksApp(): void {
  if (!managerTasksAppUrl) return;
  window.open(managerTasksAppUrl, "_blank", "noopener");
}
...
export function initDashboard(opts: DashboardOptions): void {
  ...
  managerTasksAppUrl = opts.tasksAppUrl || "";
...
const BASELINE_EMPLOYEES_TEMPLATE: JourneyEmployeeEntry[] = [
  ...
  { name: "Natalie Cho", initials: "NC", journeyName: "Inter-Location Transfer", stepIndex: 1, stepName: "Receiving Location Confirmation", totalSteps: 4, completed: false, daysOnStep: 6 },

  // AGM/GM Leadership Pipeline
  { name: "Daniel Esposito", initials: "DE", journeyName: "AGM/GM Leadership Pipeline", stepIndex: 2, stepName: "Store P&L Training", totalSteps: 4, completed: false, daysOnStep: 1 },
...
function initTeamTasksAndRoleChange(
  ...
  const BASELINE_TASKS: TaskGroup[] = demoMode
    ? [
        ...
        { name: "Chris Diaz", tasks: [{ title: "Complete product safety certification renewal", priority: "Priority_2", status: "Open" }] },
      ]
    : [];
...
interface Worktags {
  costCenter: string; // e.g. "CC-4102 · Springfield – Facilities"
  spendCategory: string; // e.g. "SC-REPAIRS-BLDG · Repairs & Maintenance – Building"
  location: string; // e.g. "Acme Corp – Springfield"
}
...
interface Requisition {
  ...
  assignedTeam?: string; // routing target, e.g. "Facilities & Engineering Team"
}
...
// Facilities Operations routing target — the same real client group the
// audit widget routes facility-ops tasks to (matched by title at runtime).
const FACOPS_GROUP_TITLE = "Facilities & Engineering Team";

// Option lists for the create form — Worktag values keep Workday vocabulary.
const REQ_COST_CENTERS = [
  "CC-4102 · Springfield – Facilities",
  "CC-4101 · Springfield – Housekeeping",
  "CC-4108 · Springfield – Warehouse",
  "CC-4110 · Springfield – Sales Floor",
  "CC-4120 · Springfield – Café",
];
const REQ_SPEND_CATEGORIES = [
  ...
  "SC-WAREHOUSE-SUPP · Warehouse Supplies",
  "SC-SUPPLIES-JAN · Janitorial & Cleaning Supplies",
  "SC-SUPPLIES-OFFICE · Office & Admin Supplies",
];
const REQ_LOCATIONS = ["Acme Corp – Springfield", "Acme Corp – Riverside", "Acme Corp – Lakeside"];
...
const REQUISITION_STORAGE_KEY = "manager-hub:requisitions";
...
// Realistic sample facilities requisitions across the status range so
...
const BASELINE_REQUISITIONS: Requisition[] = [
  {
    ...
    worktags: {
      costCenter: "CC-4102 · Springfield – Facilities",
      spendCategory: "SC-INSPECTIONS · Safety & Compliance Inspections",
      location: "Acme Corp – Springfield",
    },
    status: "Draft",
    currency: "USD",
  ...
  },
  {
    ...
    supplier: "Lincoln Supply Co.",
    worktags: {
      costCenter: "CC-4108 · Springfield – Warehouse",
      spendCategory: "SC-WAREHOUSE-SUPP · Warehouse Supplies",
      location: "Acme Corp – Springfield",
    },
    status: "Successfully Completed",
    currency: "USD",
    memo: "Monthly warehouse supply restock.",
    chainStage: "Received",
    purchaseOrderNumber: "PO-WD-2026-2087",
    lines: [
      { lineType: "Goods", itemDescription: "Shrink wrap (5000 ft roll)", spendCategory: "Warehouse Supplies", quantity: 6, unitOfMeasure: "Roll", unitCost: 189.0, extendedAmount: 1134.0 },
      { lineType: "Goods", itemDescription: "Packing tape (case of 36)", spendCategory: "Warehouse Supplies", quantity: 4, unitOfMeasure: "Case", unitCost: 42.5, extendedAmount: 170.0 },
    ],
  },
  {
    ...
    supplier: "Grainger Industrial Supply",
    worktags: {
      costCenter: "CC-4101 · Springfield – Housekeeping",
  ...
  },
  {
    ...
    chainStage: "Requisition",
    lines: [
      { lineType: "Service", itemDescription: "Break-room wayfinding signage — replace 6 panels", spendCategory: "Repairs & Maintenance – Building", extendedAmount: 540.0 },
    ],
  },
];
...
const BASELINE_MY_TASKS: MyTaskEntry[] = [
  ...
  { title: "Approve Q3 training room schedule", priority: "Priority_2", status: "Open" },
...
const ROLE_CHANGE_STORAGE_KEY = "manager-hub:role-changes";
...
// a paired employee-facing widget — same "always populated" rule as
// BASELINE_REQUISITIONS. shiftId targets the featured employee's own
// Training Room shift 2 days out (matching OPEN_TRAINING_SHIFT_DAY_OFFSETS'
// seed pattern in shift-data.ts), so approving it behaves exactly like a
// real one.
function baselineShiftRequests(): ShiftChangeRequest[] {
  const featured = featuredEmployeeName();
  return [
    {
      id: "baseline-featured-swap",
      shiftId: `${featured.replace(/\s+/g, "-").toLowerCase()}-${isoDateOffset(2)}`,
      requestedBy: featured,
      type: "Swap",
...
function initShiftRoster(
  ...
  window.addEventListener("storage", (e) => {
    if (!e.key || e.key.startsWith("manager-hub:")) refresh();
  });
